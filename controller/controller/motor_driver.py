...
class MotorDriver(Node):
    ...
    def motor_command_callback(self, motor_command):
        if motor_command.is_pwm:
            # If PWM is being used, directly send PWM values
            self.send_pwm_motor_command(motor_command.mot_1_req_rad_sec, motor_command.mot_2_req_rad_sec)
        else:
            # Convert linear/angular velocity to motor PWM values
            linear_velocity = motor_command.linear_velocity
            angular_velocity = motor_command.angular_velocity

            # Convert to PWM values for each motor
            mot_1_pwm = linear_velocity - angular_velocity
            mot_2_pwm = linear_velocity + angular_velocity

            # Limit the PWM values to [-250, 250] range
            mot_1_pwm = max(min(mot_1_pwm, 250), -250)
            mot_2_pwm = max(min(mot_2_pwm, 250), -250)

            # Send PWM motor commands
            self.send_pwm_motor_command(mot_1_pwm, mot_2_pwm)
